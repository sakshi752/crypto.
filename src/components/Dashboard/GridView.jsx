  ...
  return (
    <div className='flex justify-center items-center p-4 py-10'>
      <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4  gap-8'>
        {coins.map(coin => (
          <div key={coin.id} className={`h-[300px] w-[300px] dark:bg-gray-800 bg-gray-300 px-5 py-4 rounded-lg shadow-lg transform transition duration-500 hover:scale-105  ${coin.price_change_percentage_24h > 0 ? "hover:border-green-500" : "hover:border-red-500"} hover:border-2 cursor-pointer flex flex-col gap-4 justify-evenly`}>
            ...
            <div>
              <p className='text-gray-400 text-sm'>Total Volume : ${coin.total_volume.toLocaleString()}</p>
              <p className='text-gray-400 text-sm'>Market Cap : ${coin.market_cap.toLocaleString()}</p>
            </div>
          </div>
        ))}
      </div>
